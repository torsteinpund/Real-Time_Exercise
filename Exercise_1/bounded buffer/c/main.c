...
struct BoundedBuffer* buf_new(int size) {
    ...
    // Initialiser navngitte semaforer
    buf->capacity = sem_open("/capacity_sem", O_CREAT, 0644, size);
    buf->numElements = sem_open("/numElements_sem", O_CREAT, 0644, 0);

    if (buf->capacity == SEM_FAILED || buf->numElements == SEM_FAILED) {
